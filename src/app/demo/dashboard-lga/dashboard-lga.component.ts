...
@Component({
  selector: 'app-dashboard-lga',
  imports: [SharedModule, ChartModule, NgApexchartsModule],
  templateUrl: './dashboard-lga.component.html',
  styleUrl: './dashboard-lga.component.scss'
})

export class DashboardLgaComponent implements  OnInit{
  ...
  lgaLabels:any = [];
  lgaData:any = [];
  authUser:number = this.apiService.getItem('uuid');
  ...
  ngOnInit() {
    ...
    this.authUser = this.apiService.getItem('uuid');
    this.options = {
      responsive: false,
  ...
  loadPropertyDistributionByZones(year){
    this.apiService.get(`billing/property-distribution/${this.authUser}/${year}`).subscribe((result:any)=>{
      this.subZoneLabels = [];
      this.subZoneData = [];
  ...
  loadBillChart(year){
    this.apiService.get(`billing/chart-summary/${this.authUser}/${year}`).subscribe((result:any)=>{
      this.labelData = [];
      this.labels = [];
  ...
  fetchMonthlyBillAmount(year){
    this.apiService.get(`chart-record/${this.authUser}/${year}`).subscribe((result:any)=>{
  ...
  loadDashboardContent(year){
    this.apiService.get(`dashboard/statistics/${this.authUser}/${year}`).subscribe((result:any)=>{
      ...
      this.lgaName = result.data.lgaName;
      this.apiService.storeUserData("lgaName", this.lgaName);
      this.sales.push(
        {
          title: 'Bill Amount',
